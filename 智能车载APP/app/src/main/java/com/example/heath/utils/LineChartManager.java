...
public class LineChartManager {
    ...
    public LineChartManager(LineChart mLineChart) {
        ...
        leftAxis.setAxisLineColor(Color.GREEN); //Y轴颜色
     ...
     * @param colours
     */
    public void showLineChart(List<Float> xAxisValues, List<List<Float>> yAxisValues, List<String> labels, List<Integer> colours,boolean mode,boolean temp) {
        initLineChart();
        ArrayList<ILineDataSet> dataSets = new ArrayList<>();
        for (int i = 0; i < yAxisValues.size(); i++) {
            ...
            //设置显示值的字体大小
            lineDataSet.setValueTextSize(9f);
            initLineDataSet(lineDataSet, colours.get(i), mode);
            dataSets.add(lineDataSet);
        }
     ...
     * @param red
     */
    public void setLowLimitLine(int low, String name, int red) {
        if (name == null) {
            name = "低限制线";
        }
